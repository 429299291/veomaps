...
import { fenceType, lockOperationWay, vehicleType, violateType, limitType} from "@/constant";
...
class RideDetail extends PureComponent {
  ...
  formatTime(val) {
    return moment(val).format("YYYY-MM-DD HH:mm:ss ") + Intl.DateTimeFormat().resolvedOptions().timeZone
  }
  ...
  formatValue = (key) => {
    ...
    const value = ride[key];
      ...
      switch (key) {
        ...
        case 'end':
          return ride.end ? ride.minutes : moment().diff(moment(ride.start), 'minutes');
        default:
          return value;
      }
  }

...
export default class TabController extends React.Component {
	...
	remove = (targetKey) => {
		if(this.state.panes.length === 1) {
			message.warning('Can\'t close last window.');
			return;
		}
		...
		return (
			<div>
				<Tabs
					...
					className={styles.tabController}

					style={{marginBottom: "0px"}}
				>
					{this.state.panes.map(pane => <TabPane tab={pane.name}  key={pane.key} style={{ marginBottom: "0px", overflowY: "visible"}}>
                        {pane.component}
                    </TabPane>)}
				</Tabs>
			</div>
